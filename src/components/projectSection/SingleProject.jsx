...
const SingleProject = ({name,year,language,align,image,link}) => {
  return (
    <motion.div 
      ...
      className={`flex w-full sm:flex-col-reverse items-center gap-8 ${align === "left" ? "md:flex-row" : "md:flex-row-reverse"} justify-end sm:flex-col`}>
        <div>
            ...
            <h3 className={`text-xl font-thin text-white font-special sm:text-center ${align === "left" ? "md:text-right" : "md:text-left"}`}>{year}</h3>
            <h3 className={`text-xl font-thin text-cyan font-special sm:text-center ${align === "left" ? "md:text-right" : "md:text-left"}`}>{language}</h3>
